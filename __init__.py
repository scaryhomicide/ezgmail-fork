...
class GmailMessage:
    ...
    def __init__(self, messageObj):
        ...
        #print(messageObj.get('payload').get('parts'))
        if "body" in str(messageObj):
            # for header in messageObj['payload']['headers']:
            #    if header['name'].upper() == 'CONTENT-TYPE':
            #        emailEncoding = _parseContentTypeHeaderForEncoding(header['value'])
            self.originalBody = base64.urlsafe_b64decode(messageObj["payload"]["parts"][0]["parts"][0]["body"]["data"]).decode(emailEncoding)
